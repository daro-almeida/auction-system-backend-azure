...
public class Cosmos {
	...
	public static Result<UpdateAuctionBidResult, ServiceError> tryUpdateAuctionBid(
		...
		while (true) {
			...
			logger.warning("Conflicting bid has id " + auction.getWinnerBidId());
			var newHighestBidDaoOpt = getBid(bidContainer, auction.getWinnerBidId());
			if (newHighestBidDaoOpt.isEmpty()) {
				logger.warning("Auction has a winner bid id but it does not exist");
				logger.warning("Auction\n" + auction);
				return Result.err(ServiceError.INTERNAL_ERROR);
			}

			var newHighestBidDao = newHighestBidDaoOpt.get();
