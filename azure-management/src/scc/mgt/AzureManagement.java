...
public class AzureManagement {
	...
	static final String MY_SUFFIX = "bdd"; // Add your suffix here
	
	static final String AZURE_COSMOSDB_NAME = "scc2223" + MY_SUFFIX;	// Cosmos DB account name
	static final String AZURE_COSMOSDB_DATABASE = "scc2223db" + MY_SUFFIX;	// Cosmos DB database name
	static final String[] BLOB_CONTAINERS = { "userimages", "auctionimages" };	// Containers to add to the blob storage
	...
	static final String[] AZURE_RG_REGIONS = Arrays.stream(REGIONS)
			.map(reg -> "scc2223-rg-" + reg.name() + "-" + MY_SUFFIX).toArray(String[]::new);
	...
	static final String[] AZURE_APP_NAME = Arrays.stream(REGIONS).map(reg -> "scc2223app" + reg.name() + MY_SUFFIX)
			.toArray(String[]::new);
	...
	public synchronized static void dumpStorageKey(Map<String, String> props, String propFilename,
		...
		synchronized (props) {
			props.put("AZURE_BLOB_STORAGE_CONNECTION_STRING", key);

		}

		synchronized (AzureManagement.class) {
			Files.write(Paths.get(propFilename), ("AZURE_BLOB_STORAGE_CONNECTION_STRING=" + key + "\n").getBytes(),
	...
	public synchronized static void dumpCosmosDBKey(Map<String, String> props, String propFilename,
		...
		synchronized (AzureManagement.class) {
			Files.write(Paths.get(propFilename),
					("AZURE_COSMOS_DB_KEY=" + account.listKeys().primaryMasterKey() + "\n").getBytes(),
					StandardOpenOption.APPEND);
			Files.write(Paths.get(propFilename), ("AZURE_COSMOS_DB_URL=" + account.documentEndpoint() + "\n").getBytes(),
					StandardOpenOption.APPEND);
			Files.write(Paths.get(propFilename), ("AZURE_COSMOS_DB_DATABASE=" + databaseName + "\n").getBytes(),
					StandardOpenOption.APPEND);
		}
		synchronized (props) {
			props.put("AZURE_COSMOS_DB_KEY", account.listKeys().primaryMasterKey());
			props.put("AZURE_COSMOS_DB_URL", account.documentEndpoint());
			props.put("AZURE_COSMOS_DB_DATABASE", databaseName);
		}
	...
	public synchronized static void dumpRedisCacheInfo(Map<String, String> props, String propFilename, 
		...
		synchronized (AzureManagement.class) {
			Files.write(Paths.get(propFilename), ("AZURE_REDIS_KEY=" + redisAccessKey.primaryKey() + "\n").getBytes(),
					StandardOpenOption.APPEND);
			Files.write(Paths.get(propFilename), ("AZURE_REDIS_URL=" + cache.hostName() + "\n").getBytes(),
					StandardOpenOption.APPEND);
		}
		synchronized (props) {
			props.put("AZURE_REDIS_KEY", redisAccessKey.primaryKey());
			props.put("AZURE_REDIS_URL", cache.hostName());
		}
		StringBuffer cmd = new StringBuffer();
		if (appName != null) {
			cmd.append("az functionapp config appsettings set --name ");
